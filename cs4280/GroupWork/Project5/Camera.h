...
// Camera class
 ...
 class Camera{
  ...
  public:
	...
	void drawGridLines(bool toggle); //draw gridlines
	void vAngle(float val);
	void aRatio (float val);
	void roll(float angle); // roll it
	void pitch(float angle); // increase pitch
	void yaw(float angle); // yaw it
	void rotate(Vector3 axis, float angle); //rotate it
	bool equals(Vector3 v1, Vector3 v2); //compare function
	void slide(float delU, float delV, float delN); // slide it
	Vector3 getV(char vector); //get a Vector
