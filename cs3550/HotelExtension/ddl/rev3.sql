...
( EventStaffID	smallint	IDENTITY(1,1)	NOT NULL,
  ...
  StaffHourlyRate   smallmoney  NOT NULL
)
...
( TicketID	smallint	IDENTITY(1,1)	NOT NULL,
  ...
  TicketPurchaseDate    smalldatetime   NOT NULL DEFAULT GetDate(),
...
( PaycheckID    smallint    IDENTITY(1,1)   NOT NULL,
  PaycheckAmount    smallmoney  NOT NULL,
  PaycheckHoursWorked   tinyint    NOT NULL,
  EventStaffID  smallint    NOT NULL
)
...
ALTER TABLE Paycheck
ADD CONSTRAINT FK_PaycheckMustHaveEventStaff
FOREIGN KEY (EventStaffID) REFERENCES EventStaff(EventStaffID)
ON UPDATE CASCADE
ON DELETE CASCADE