void createVector()
{
    ...
    myVect.clear();                 // Clears vector for use
    // Inserts random values into vector
    for(int c = 0; c < 5000; c++)
    {
        x = rand() % 512;       // I chose 512 for no particular reason
        myVect.push_back(x);
    }
}

void insertVector(int loc)
{
    createVector(); // I know it recreates it everytime, but it's ok in this case
    timer t1;
    
    // Inserts a number at the beginning of a vector
    if(loc == 0)
    {
        vector<int>::iterator c = myVect.begin();
    
        t1.start();
        myVect.insert(c,1,rand() % 512);
        myVect.insert(c,1,rand() % 512);
        myVect.insert(c,1,rand() % 512);
        t1.stop();
    ...
    else if(loc == 1)
    {
        ...
        t1.start();
        myVect.insert(c + (myVect.size() / 2),1,rand() % 512);
        myVect.insert(c + (myVect.size() / 2),1,rand() % 512);
        myVect.insert(c + (myVect.size() / 2),1,rand() % 512);
        t1.stop();
...
void walkVector(int loc)
{
    ...
    else if(loc == 1)
    {
        ...
        t1.start();
        for(i = myVect.rbegin(); i != myVect.rend(); ++i) 
            // cout << *i;
            int temp = *i;      // Clean up output
        for(i = myVect.rbegin(); i != myVect.rend(); ++i) 
            // cout << *i;
